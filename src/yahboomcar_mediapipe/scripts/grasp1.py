...
class YOLOv8ArmController:
    def __init__(self):
        ...
        self.z_limits = (0, 400)      # z范围：0～400
        
        # ======================
        # 3. 夹爪控制参数
        # ======================
        self.gripper_open_angle = 1.08   # 夹爪张开角度
        ...
        # 平移向量
        self.T = np.array([0.52, -0.03, 0.25])
    ...
    def initialize_arm_position(self):
        """
        发送机械臂初始化位置指令
        {"T":102,"base":0,"shoulder":-1.19,"elbow":2.46,"hand":3.14,"spd":10,"acc":10}
        """
        if self.ser is None:
    ...
    def execute_grasp_sequence(self, target_x, target_y, target_z):
        ...
        # 2. 运动到目标位置后3cm处
        approach_x = target_x - self.grasp_offset
        is_valid, error_msg = self.check_arm_limits(approach_x, target_y, target_z)
        if not is_valid:
            rospy.logerr(f"接近位置超限: {error_msg}")
            return False
            
        rospy.loginfo(f"2. 运动到接近位置: X={approach_x:.1f}, Y={target_y:.1f}, Z={target_z:.1f}")
        if not self.send_arm_command(approach_x, target_y, target_z, spd=0.15):
            return False
        ...
        # 4. 前进到目标位置
        rospy.loginfo(f"4. 前进到目标位置: X={target_x:.1f}, Y={target_y:.1f}, Z={target_z:.1f}")
    ...
    def camera_to_arm_coordinates(self, point_camera):
        """
        将相机坐标系下的点转换到机械臂坐标系
        point_camera: [x, y, z] 在相机坐标系下的坐标
        返回: [x, y, z] 在机械臂坐标系下的坐标
        """
        point_camera = np.array(point_camera).reshape(3, 1)
        
        # 坐标系转换: P_arm = R * P_camera + T
        point_arm = self.R @ point_camera + self.T.reshape(3, 1)
        
        return point_arm.flatten().tolist()
    ...
    def detect_callback(self, msg):
        ...
        try:
            ...
            rospy.loginfo(
                f"机械臂坐标系坐标=(X={arm_coords[0]:.3f}, Y={arm_coords[1]:.3f}, Z={arm_coords[2]:.3f}) [米]"
            )

            # 转换为毫米
            target_x = arm_coords[0] * 1000
            target_y = arm_coords[1] * 1000
            target_z = arm_coords[2] * 1000
